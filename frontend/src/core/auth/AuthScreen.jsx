...
const PreviewPanel = ({ previews, chessPreviews }) => {
  ...
  return (
    <div className="flex flex-col mt-8 w-full relative z-10">
      ...
      {!hasData ? (
        <div className="flex flex-col items-center justify-center p-8 bg-black/10 rounded-[2rem] border border-white/10 text-center mt-8 backdrop-blur-sm self-start min-w-[240px]">
        ...
        <div className="w-full flex justify-center">
          <Podium mode={tab} top3={listData.slice(0, 3)} align="center" className="mt-24 sm:mt-28 mb-2" isWidget={true} />
        </div>
      )}

      {listData.length > 3 && (
        <div className="mt-6 flex flex-col gap-3 pl-2 max-w-[340px]">
            ...
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  ref={passwordRef} 
                  placeholder="••••••••" 
                  className="w-full bg-slate-50 border-2 border-slate-100 p-3 sm:p-3.5 pr-12 rounded-[1.25rem] font-bold text-slate-800 text-sm outline-none focus:border-[#009660] transition-all" 
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#009660] rounded-full p-1 transition-colors"
