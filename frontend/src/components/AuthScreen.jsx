...
const AuthScreen = ({ onAuthSuccess, onGuestStart, onJoinRoom }) => {
  ...
  return (
    <div className="min-h-screen bg-[#009660] flex flex-col items-center justify-center p-6 font-sans overflow-hidden relative">
      ...
      <header className="flex flex-col items-center flex-shrink-0 z-10 pt-4 sm:pt-0 mb-6 w-full">
      ...
      <div className="bg-white p-8 sm:p-10 rounded-[3.5rem] shadow-[0_25px_80px_rgba(0,0,0,0.3)] w-full max-w-sm flex flex-col animate-in slide-in-from-bottom-8 duration-500 z-10 border-b-[12px] border-emerald-900/10 mb-8">
        
        <h2 className="text-3xl font-black text-emerald-900 uppercase italic mb-6 tracking-tight text-center">
          {view === "register" ? "Fazer Cadastro" : "Entrar no Jogo"}
        </h2>
        ...
        <form onSubmit={view === "register" ? handleRegister : handleLogin} className="space-y-3">
          ...
          <button
            disabled={loading}
            className="w-full bg-[#FFCE00] text-[#009660] py-5 rounded-2xl font-black text-lg uppercase tracking-wider shadow-[0_8px_0_#d1a900] hover:translate-y-1 hover:shadow-[0_4px_0_#d1a900] active:translate-y-2 active:shadow-none transition-all cursor-pointer mt-4 shadow-lg disabled:opacity-50"
          >
        ...
        <button
          ...
          className="text-emerald-900/60 hover:text-emerald-900 font-black text-[10px] uppercase tracking-widest transition-colors py-4 text-center mt-2 group"
        >
          {view === "login" ? (
            <>Não tem conta? <span className="text-[#009660] group-hover:underline">Registrar-se</span></>
          ) : (
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="bg-emerald-50 text-emerald-600 py-4 px-8 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-emerald-100 transition-all active:scale-95 cursor-pointer border-2 border-emerald-100"
          >
            JOGAR SEM CONTA 🎮
          </button>
          ...
          <button
            onClick={onJoinRoom}
            className="bg-amber-50 text-amber-600 py-4 px-8 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-amber-100 transition-all active:scale-95 cursor-pointer border-2 border-amber-100"
          >
            ENTRAR EM SALA 🔑
          </button>
        </div>

        <p className="text-[9px] text-center font-black text-emerald-900/30 uppercase tracking-widest mt-4">
