...
const TournamentEditModal = ({ tournament, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (tournament) {
      setFormData({
        ...
        startsAt: tournament.startsAt ? new Date(tournament.startsAt).toISOString().split('T')[0] : '',
        endsAt: tournament.endsAt ? new Date(tournament.endsAt).toISOString().split('T')[0] : ''
      });
      
      const presetSizes = [4, 8, 16, 32];
      if (!presetSizes.includes(tournament.maxPlayers)) {
        setCustomPlayers(tournament.maxPlayers.toString());
      }
    }
  }, [tournament]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || 'Erro ao editar campeonato');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-emerald-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[2rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-emerald-100 shadow-2xl">
        <div className="sticky top-0 bg-white p-6 md:p-8 border-b border-emerald-50 flex justify-between items-center z-10">
          <div>
            <h2 className="text-2xl font-black text-emerald-900 uppercase italic tracking-tighter">Editar Campeonato</h2>
            <p className="text-[10px] font-black text-emerald-900/60 uppercase tracking-widest mt-1">Modificar Torneio Oficial</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="text-[10px] font-black text-emerald-900/60 uppercase tracking-widest">Número Máx. de Participantes *</label>
              ...
              <p className="text-[9px] text-emerald-900/40 uppercase font-black tracking-widest">O número precisa ser par e não menor que o total atual de inscritos.</p>
            </div>
          ...
          <div className="pt-6 border-t border-emerald-50 flex justify-end gap-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-[#009660] text-white px-8 py-3 rounded-2xl font-black uppercase tracking-widest hover:bg-emerald-700 transition-colors shadow-lg active:scale-95 disabled:opacity-50"
            >
