...
class VelhaRankingService {
  /**
   * Atualiza o ranking após uma partida de Xadrez da Velha
   */
  async updateRanking(whiteId, blackId, result) {
    ...
    try {
      ...
      console.log(`🏆 [Velha Ranking] Atualizado para ${whiteUser.fullName} e ${blackUser.fullName}`);
    } catch (error) {
      console.error('❌ [VelhaRankingService] Erro ao atualizar ranking:', error);
    }
  }
  ...
  async getStudentRanking() {
    try {
      const prisma = getPrisma();
      const leaders = await prisma.velhaRanking.findMany({
        where: { points: { gt: 0 } },
        orderBy: { points: 'desc' },
        take: 100
      });
      ...
      console.error('❌ [VelhaRankingService] Erro ao buscar ranking de alunos:', error);
      return [];
    }
  }
  ...
  async getSchoolRanking() {
    try {
      ...
      const rankings = await prisma.velhaRanking.findMany({
      ...
      console.error('❌ [VelhaRankingService] Erro ao buscar ranking de escolas:', error);
      return [];
    }
  }
  ...
  /**
   * Preview para tela de login/lobby
   */
...
module.exports = new VelhaRankingService();
